...
/**
 * Unit test of {@link WordCountDemoCustom} stream using TopologyTestDriver.
 */
public class WordCountDemoCustomTest {
    ...
    public void setup() throws IOException {
        ...
        WordCountDemoCustom.createWordCountStream(builder);
        testDriver = new TopologyTestDriver(builder.build(), WordCountDemoCustom.getStreamsConfig(null));
        inputTopic = testDriver.createInputTopic(WordCountDemoCustom.INPUT_TOPIC, new StringSerializer(), new StringSerializer());
        outputTopic = testDriver.createOutputTopic(WordCountDemoCustom.OUTPUT_TOPIC, new StringDeserializer(), new LongDeserializer());
    }
    ...
    public void testGetStreamsConfig() throws IOException {
        ...
        try {
            Properties config = WordCountDemoCustom.getStreamsConfig(new String[] {tmp.getPath()});
            assertThat("localhost:1234", equalTo(config.getProperty(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG)));

            config = WordCountDemoCustom.getStreamsConfig(new String[] {tmp.getPath(), "extra", "args"});
            assertThat("localhost:1234", equalTo(config.getProperty(CommonClientConfigs.BOOTSTRAP_SERVERS_CONFIG)));
        } finally {
